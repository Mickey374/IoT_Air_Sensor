...
static void post_state_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset){
    ...
    int validate = 1;
     ...
     if((len = coap_get_post_variable(request, "value", &value))) {
         ...
         // If value is 1 and the LED is currently inactive or the intensity is less than 100, increase intensity by 10 and activate LED 
         if(atoi(value) == 1){
         ...
         // If value is 0 and the LED is active, set isActive to false, intensity to 10 and activate LED 
